...
for j=20:(20+l-1)
    for i=20:(20+w-1)
      if isnan(tmp_pos_map(j, i))~=1
        r=1;
        ratio=10; %the initial arbitrary chosen value
        while (r<ratio)&&(j>r+1)&&(i>r+1)&&(i+r+1<=size(tmp_spk_map,2))&&(j+r+1<=size(tmp_spk_map,1))
          r=r+1;
          ...
          tmp1=reshape(tmp1, 1, size(tmp1, 1)*size(tmp1, 2));
        ...
        tmp_smooth_r_map(j, i)=s/n*pos_sample_rate;
