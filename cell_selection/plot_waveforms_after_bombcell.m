...
%     ipdf = 1;
    ...
    array_noise_for_display = cell(1,length(find(unitType==0 | unitType==3)));
    units_good_for_array = [];
    units_noise_for_array = [];


    disp('storing waveforms...');
    for cell_i = cluster_indices' % 1:max(npy_spike_clusters)%1:1593 %cluster_list_after_manual_cutting

        disp(strcat(num2str(find(cluster_indices'==cell_i)),'/',num2str(length(cluster_indices'))));
...
%                 title(num2str(cell_i));
            ...
            if unitType(find(cluster_indices==cell_i)) == 1 || unitType(find(cluster_indices==cell_i)) == 2 % if this unit was classified as a MUA or single unit
                units_good_for_array = [units_good_for_array,cell_i];
                array_waveforms_for_display{1,length(units_good_for_array)} = array_temp;
            elseif unitType(find(cluster_indices==cell_i)) == 0 || unitType(find(cluster_indices==cell_i)) == 3 % if this unit was classified as noise or axonal unit
