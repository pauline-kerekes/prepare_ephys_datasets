...
%                     title(num2str(units_good_for_array(1,cell_i)));
        ...
        % remove the false positive units
        new_cells = all_cells;
...
%                     xticks([]);
    ...
    %     best_channels = best_channels_bombcell(ismember(cluster_indices,new_cells));
        ...
        range_descr=strcat('A',num2str(2),':','A',num2str(1+length(new_cells)));
        writematrix(new_cells',cell_list1,'Sheet',1,'Range',char(range_descr));

        range_descr=strcat('E',num2str(2),':','E',num2str(2+length(new_cells)));
        writematrix(manual_curation_empty',cell_list1,'Sheet',1,'Range',char(range_descr));
        
        % save the n false negative, n false positive and total n cells (not
        % counting axonal classified)
        save('performance_classification.mat','false_negatives','false_positives','units_noise_for_array','units_good_for_array');
        

        %writematrix([new_cells',manual_curation_empty'],filename);
    else
        disp('the cells are already selected for that session');
    end
