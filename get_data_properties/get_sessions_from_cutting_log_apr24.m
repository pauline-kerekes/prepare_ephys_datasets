...
function [list_animals_cut_log,list_sessions_cut_log,list_proj_cut_log,list_kilo_cut_log,list_probe_cut_log,list_shankmix_log,list_protocols_log,list_VE_indices_log,list_npixels_log] = get_sessions_from_cutting_log_apr24(path_to_cutting_log, mouse)
    ...
    colprobe = find(first_line==string('neuropixel probe version'));
    ...
    for i_line = 2:size(file_exp,1) % i do a loop instead of just indexing to check the session names - some miss a '0' 
        %at the beginning when they are read from excel and I want to correct for that.
            ...
            if string(file_exp(i_line,column_ok)) == string('y')
                    ...
                    list_probe_cut_log = [list_probe_cut_log cell2mat(file_exp(i_line,colprobe))];
