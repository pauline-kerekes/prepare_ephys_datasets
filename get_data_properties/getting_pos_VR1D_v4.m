...
function [vector_lat] = getting_pos_VR1D_v4(posfile,folder_name,session)
...
for i = 11 : size(A) 
    ...
    if A(i)== 998 
          ...
          vector_lat_tmp = [vector_lat_tmp A(i-8)];
...
%if s(end) ~= '8'
    ...
    vector_lat = vector_lat_tmp(ind:end);
%else 
    %vector_vertical_pos = [];
    %vector_rewards = [];
    %vector_feeder = [];
...
%
vector_lat = interp1(vector_time,vector_lat,1:20:max(vector_time));
